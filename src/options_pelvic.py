...
class TrainOptions():
  def __init__(self):
    ...
    self.parser.add_argument('--checkpoint_dir', type=str, default='../results', help='path for checkpoint files')
    self.parser.add_argument('--result_dir', type=str, default='../results', help='path for saving result images')
    self.parser.add_argument('--display_freq', type=int, default=1000, help='freq (iteration) of display')
    self.parser.add_argument('--img_save_freq', type=int, default=100000, help='freq (epoch) of saving images')
    self.parser.add_argument('--model_save_freq', type=int, default=100000, help='freq (epoch) of saving models')
    self.parser.add_argument('--no_display_img', action='store_true', help='specified if no dispaly')

    # training related
    self.parser.add_argument('--max_iter', type=int, default=100000, help='max training iterations')
    ...
    self.parser.add_argument('--aug_sigma', type=float, default=0.5, help="augmentation parameter:sigma")
    self.parser.add_argument('--aug_points', type=float, default=20, help="augmentation parameter:points")
    self.parser.add_argument('--aug_rotate', type=float, default=0, help="augmentation parameter:rotate")
    self.parser.add_argument('--aug_zoom', type=float, default=1., help="augmentation parameter:zoom")
    self.parser.add_argument('--psnr_threshold', type=float, default=18.0, help="only save the checkpoint file when PSNR reach this threshold")
    self.parser.add_argument('--do_validation', type=int, default=1, help="whether do validation during training")
    self.parser.add_argument('--mini', type=int, default=0, help="whether do mini data to avoid memory insufficient issue")
  def parse(self):
    self.opt = self.parser.parse_args()

    if self.opt.mini:
        self.opt.resize_size = self.opt.resize_size // 2
        self.opt.crop_size = self.opt.crop_size // 2
...
class TestOptions():
  def __init__(self):
    ...
    self.parser.add_argument('--checkpoint_dir', type=str, default='../checkpoints', help='path for saving result images and models')
    ...
    self.parser.add_argument('--gpu', type=int, default=0, help='gpu')
    self.parser.add_argument('--mini', type=int, default=0, help='mini')
